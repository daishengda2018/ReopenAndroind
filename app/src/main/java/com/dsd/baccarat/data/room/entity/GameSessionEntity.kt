 ...
 * @property endTime 游戏结束时间（初始为 null）
...
data class GameSessionEntity(
    ...
    @ColumnInfo(index = true)
    var endTime: Long? = null, // 游戏结束时间（初始为 null）
    @ColumnInfo(index = true)
    var isActive: Boolean = true, // 标记是否活跃
) {
    companion object {
        fun create() = GameSessionEntity(UUID.randomUUID().toString(), System.currentTimeMillis(), null, true)
    }
}
